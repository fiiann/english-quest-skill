...
def check_daily_reset(state: dict) -> dict:
    ...
    # Backward-compat: ensure vocab key exists even if skill_components was initialized before vocab was added
    if "vocab" not in state["skill_components"]:
        state["skill_components"]["vocab"] = {"xp": 0, "total_score": 0, "count": 0}
...
def start_vocab_session(state: dict, topic: str = None) -> dict:
    """Start a new vocab session of 10 cards."""
    player = state["player"]
    ...
    # Update last_practice_date so it counts as a practice day
    player["last_practice_date"] = date.today().isoformat()

    # Update vocab-specific date
    vs["last_vocab_date"] = date.today().isoformat()
    ...
    if len(cards) < 10:
        ...
        for t in (list(VOCAB_BANK.keys()) if topic is None else [topic]):
            for card in VOCAB_BANK.get(t, []):
                if card["id"] not in seen_ids and len(cards) < 10:
                    c = dict(card)
                    c["_topic"] = t
                    cards.append(c)
                    seen_ids.add(card["id"])
